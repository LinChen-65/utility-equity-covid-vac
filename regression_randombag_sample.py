...
history_D2_no_vaccination = np.fromfile(os.path.join(root,MSA_NAME,'vaccination_results_adaptive_%s_0.01'% VACCINATION_RATIO,
...
randombag_results['Avg_Essential_Worker_Ratio'] = randombag_results.apply(lambda x: get_avg_feat(x['Vaccinated_Idxs'],data,'Essential_Worker_Ratio'), axis=1)
...
randombag_results['Std_Essential_Worker_Ratio'] = randombag_results.apply(lambda x: get_std_feat(x['Vaccinated_Idxs'],data,'Essential_Worker_Ratio'), axis=1)
...
print('Avg_Essential_Worker_Ratio',randombag_results['Avg_Essential_Worker_Ratio'].max(),randombag_results['Avg_Essential_Worker_Ratio'].min())
...
for sample_idx in range(NUM_SAMPLE):
    ...
    #demo_feat_list = ['Avg_Elder_Ratio','Avg_Mean_Household_Income','Avg_Essential_Worker_Ratio']
    demo_feat_list = ['Avg_Elder_Ratio','Avg_Mean_Household_Income','Avg_Essential_Worker_Ratio',
                      'Std_Elder_Ratio','Std_Mean_Household_Income','Std_Essential_Worker_Ratio']

    # Regression target
    target_list = ['Fatality_Rate_Rel','Age_Gini_Rel','Income_Gini_Rel','Occupation_Gini_Rel']

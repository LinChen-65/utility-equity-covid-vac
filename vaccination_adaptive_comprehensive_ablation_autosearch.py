...
for policy in policy_to_compare:
    ...
    if(policy=='no_vaccination'):
        ...
        if(consider_hesitancy=='False'):
            exec('history_D2_%s = np.fromfile(os.path.join(root,MSA_NAME,\'vaccination_results_adaptive_%sd_%s_%s\',\'history_D2_%s_adaptive_%sd_%s_%s_30seeds_%s\'))' 
                %(policy,VACCINATION_TIME_STR,VACCINATION_RATIO,RECHECK_INTERVAL,policy,VACCINATION_TIME_STR,VACCINATION_RATIO,RECHECK_INTERVAL,MSA_NAME))
            if(len(eval('history_D2_%s'%policy))==(2*63*NUM_SEEDS*M)):
                ...
                # Save back results
                exec('np.array(history_D2_%s).tofile(os.path.join(root,MSA_NAME,\'vaccination_results_adaptive_%sd_%s_%s\',\'history_D2_%s_adaptive_%sd_%s_%s_30seeds_%s\'))' 
                %(policy,VACCINATION_TIME_STR,VACCINATION_RATIO,RECHECK_INTERVAL,policy,VACCINATION_TIME_STR,VACCINATION_RATIO,RECHECK_INTERVAL,MSA_NAME))
            ...
            exec('history_D2_%s = np.fromfile(os.path.join(root,MSA_NAME,\'vaccination_results_adaptive_%sd_%s_%s\',\'history_D2_%s_adaptive_%sd_%s_%s_30seeds_acceptance_%s_%s\'))' 
                %(policy,VACCINATION_TIME_STR,VACCINATION_RATIO,RECHECK_INTERVAL,policy,VACCINATION_TIME_STR,VACCINATION_RATIO,RECHECK_INTERVAL,ACCEPTANCE_SCENARIO,MSA_NAME))
            exec('history_D2_%s = np.reshape(history_D2_%s,(63,NUM_SEEDS,M))'%(policy,policy))
...
#print('Demographic feature list: ', demo_feat_list)
gini_table = make_gini_table(policy_list=policy_to_compare, demo_feat_list=demo_feat_list, num_groups=NUM_GROUPS, 
                             save_path=None, save_result=False)
print('Gini table of all the other policies: \n', gini_table)

# Best results from other polices  
lowest_death_rate = float(gini_table.iloc[0].min())
lowest_age_gini = float(gini_table.iloc[2].min())
lowest_income_gini = float(gini_table.iloc[4].min())
lowest_occupation_gini = float(gini_table.iloc[6].min())

# No_Vaccination results
no_vaccination_death_rate = float(gini_table.iloc[0][0])
no_vaccination_age_gini = float(gini_table.iloc[2][0])
no_vaccination_income_gini = float(gini_table.iloc[4][0])
no_vaccination_occupation_gini = float(gini_table.iloc[6][0])
print(no_vaccination_death_rate,no_vaccination_age_gini,no_vaccination_income_gini,no_vaccination_occupation_gini)

# Baseline results
baseline_death_rate = float(gini_table.iloc[0][1])
baseline_age_gini = float(gini_table.iloc[2][1])
baseline_income_gini = float(gini_table.iloc[4][1])
baseline_occupation_gini = float(gini_table.iloc[6][1])
...
while(True):
    ...
    for i in range(int(distribution_time)):
        ...
        vulner_scores += 1
        vulner_scores /= np.max(vulner_scores)
        damage_scores += 1
        damage_scores /= np.max(damage_scores)
    ...
    policy_all = policy_to_compare+['Hybrid_Flood']
    ...
    gini_table = make_gini_table(policy_list=policy_all, demo_feat_list=demo_feat_list, num_groups=NUM_GROUPS, 
                                 save_path=None, save_result=False)
    print('Gini table of all policies: \n', gini_table)

    hybrid_death_rate = float(gini_table['Hybrid_Flood'].iloc[0])
    hybrid_age_gini = float(gini_table['Hybrid_Flood'].iloc[2])
    hybrid_income_gini = float(gini_table['Hybrid_Flood'].iloc[4])
    hybrid_occupation_gini = float(gini_table['Hybrid_Flood'].iloc[6])
    ...
    print('Comparisons:')
    print('Death rate: ', hybrid_death_rate, baseline_death_rate, 'Good enough?', better_efficiency)
    print('Age gini: ', hybrid_age_gini, baseline_age_gini, 'Good enough?', better_age_gini)
    print('Income gini: ', hybrid_income_gini, baseline_income_gini, 'Good enough?', better_income_gini)
    print('Occupation gini: ', hybrid_occupation_gini, baseline_occupation_gini, 'Good enough?', better_occupation_gini)
    ...
    #num_better_now=4 # test   
    if(num_better_now==4):
        file_savename = os.path.join(root,MSA_NAME,
                                     'vaccination_results_adaptive_%sd_%s_%s'
                                     %(VACCINATION_TIME_STR,VACCINATION_RATIO,RECHECK_INTERVAL),
                                     'history_D2_adaptive_hybrid_%sd_%s_%s_%s%s%s%s%s_%sseeds_acceptance_%s_%s'
                                     %(VACCINATION_TIME_STR,VACCINATION_RATIO,RECHECK_INTERVAL,w1,w2,w3,w4,w5,NUM_SEEDS,ACCEPTANCE_SCENARIO,MSA_NAME)
                                     )
    ...
    if(refine_mode==True):
        refine_count += 1
        if(refine_count>=refine_threshold):#4
            break 
...
policy_all = policy_to_compare+['Best_Hybrid_Flood']+['Hybrid_Ablation']
...
hybrid_ablation_death_rate = float(gini_table['Hybrid_Ablation'].iloc[0])
hybrid_ablation_age_gini = float(gini_table['Hybrid_Ablation'].iloc[2])
hybrid_ablation_income_gini = float(gini_table['Hybrid_Ablation'].iloc[4])
hybrid_ablation_occupation_gini = float(gini_table['Hybrid_Ablation'].iloc[6])
...
file_savename = os.path.join(root,MSA_NAME,'vaccination_results_adaptive_%sd_%s_%s'%(VACCINATION_TIME_STR,VACCINATION_RATIO,RECHECK_INTERVAL),
                             'history_D2_adaptive_hybrid_ablation_%sd_%s_%s_%s%s%s_%sseeds_acceptance_%s_%s'% (VACCINATION_TIME_STR,VACCINATION_RATIO,RECHECK_INTERVAL,w1,w2,w3,NUM_SEEDS,ACCEPTANCE_SCENARIO,MSA_NAME))

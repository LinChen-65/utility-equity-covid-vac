...
demo_feat_show = {'Age':'Elder Ratio',
                  'Mean_Household_Income':'Average Household Income',#'Mean Household Income',
...
pdb.set_trace()
################################################################################
# Predicted disparities among demographic groups, average version, Race
...
for msa_idx in range(len(constants.MSA_NAME_LIST)):
    ...
    # Grouping: 按NUM_GROUPS分位数，将全体CBG分为NUM_GROUPS个组，将分割点存储在separators中
    #separators = functions.get_separators(cbg_race_msa, NUM_GROUPS, 'Black_Ratio','Sum', normalized=False)
    #cbg_race_msa['Black_Ratio_Quantile'] =  cbg_race_msa['Black_Ratio'].apply(lambda x : functions.assign_group(x, separators))
    #print(cbg_race_msa[cbg_race_msa['Black_Ratio_Quantile']==1]['Sum'].sum())
    separators = functions.get_separators(cbg_race_msa, NUM_GROUPS, 'White_Ratio','Sum', normalized=False)
    cbg_race_msa['White_Ratio_Quantile'] =  cbg_race_msa['White_Ratio'].apply(lambda x : functions.assign_group(x, separators))
    #print(cbg_race_msa[cbg_race_msa['White_Ratio_Quantile']==1]['Sum'].sum())
    
    ##################### test #####################
    # Examine the average elder_ratio in each race decile
    ...
    for i in range(NUM_GROUPS):
        print(np.round(cbg_race_msa[cbg_race_msa['White_Ratio_Quantile']==i]['Elder_Ratio'].mean(), 4)) #'group ', i, ': ',
    ...
    #results = get_msa_result(cbg_table=cbg_race_msa, demo_feat='Black_Ratio', num_groups=NUM_GROUPS) #20220227
    results = get_msa_result(cbg_table=cbg_race_msa, demo_feat='White_Ratio', num_groups=NUM_GROUPS) #20220227
    count += 1
    result_dict[MSA_NAME] = copy.deepcopy(results)

return_values = get_avg_upper_lower(result_dict, num_groups=NUM_GROUPS) #20220227
avg_agnostic, avg_aware, upper_agnostic, upper_aware, lower_agnostic, lower_aware, error_agnostic, error_aware = return_values #20220227
#savepath = os.path.join(saveroot, 'groupwise_death_rate_race_black.png')
#plot_groupwise_death_rate(demo_feat='Black_Ratio', num_groups=NUM_GROUPS, alpha=alpha, markersize=markersize, save_figure=True, savepath=savepath)
savepath = os.path.join(saveroot, 'groupwise_death_rate_race_white.png')
plot_groupwise_death_rate(demo_feat='White_Ratio', num_groups=NUM_GROUPS, alpha=alpha, markersize=markersize, save_figure=True, savepath=savepath)


################################################################################
# Predicted disparities among demographic groups, average version, Ethnicity
...
for msa_idx in range(len(constants.MSA_NAME_LIST)):
    ...
    # Grouping: 按NUM_GROUPS分位数，将全体CBG分为NUM_GROUPS个组，将分割点存储在separators中
    separators = functions.get_separators(cbg_ethnic_msa, NUM_GROUPS, 'Hispanic_Ratio','Sum', normalized=False)
    cbg_ethnic_msa['Hispanic_Ratio_Quantile'] =  cbg_ethnic_msa['Hispanic_Ratio'].apply(lambda x : functions.assign_group(x, separators))
    print(cbg_ethnic_msa[cbg_ethnic_msa['Hispanic_Ratio']==0]['Sum'].sum())
    ...
    cbg_ethnic_msa['Elder_Ratio'] = cbg_age_msa['Elder_Ratio'].copy()

    for i in range(NUM_GROUPS):
        print(np.round(cbg_ethnic_msa[cbg_ethnic_msa['Hispanic_Ratio_Quantile']==i]['Elder_Ratio'].mean(), 4)) #'group ', i, ': ',
    pdb.set_trace()
    '''
    ##################### test #####################
    ...
    # Add simulation results to grouping table
    cbg_ethnic_msa['Final_Deaths_No_Vaccination'] = deaths_cbg_no_vaccination[-1,:]
    cbg_ethnic_msa['Final_Deaths_Age_Agnostic'] = deaths_cbg_age_agnostic[-1,:]
    # Deal with NaN values
    ...
    results = get_msa_result(cbg_table=cbg_ethnic_msa, demo_feat='Hispanic_Ratio', num_groups=NUM_GROUPS) #20220227
    count += 1
    result_dict[MSA_NAME] = copy.deepcopy(results)

return_values = get_avg_upper_lower(result_dict, num_groups=NUM_GROUPS) #20220227
avg_agnostic, avg_aware, upper_agnostic, upper_aware, lower_agnostic, lower_aware, error_agnostic, error_aware = return_values #20220227
savepath = os.path.join(saveroot, 'groupwise_death_rate_ethnic.png')
plot_groupwise_death_rate(demo_feat='Hispanic_Ratio', num_groups=NUM_GROUPS, alpha=alpha, markersize=markersize, save_figure=True, savepath=savepath)

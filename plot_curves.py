...
for msa_idx in range(len(msa_name_list)):
    ...
    #deaths_total_no_vac = np.mean(np.sum(history_D2_no_vac, axis=2), axis=1) 
    '''
    ...
    #deaths_total_hybrid = np.mean(np.sum(history_D2_hybrid, axis=2), axis=1) #(63,30,3130)->(63,30)->(63,)
    '''
    deaths_total_mean_hybrid = np.mean(np.sum(history_D2_hybrid, axis=2), axis=1) 
    deaths_total_max_hybrid = np.max(np.sum(history_D2_hybrid, axis=2), axis=1) 
    deaths_total_min_hybrid = np.min(np.sum(history_D2_hybrid, axis=2), axis=1) 
    # Transform into daily results
    mean_hybrid = [0]
    max_hybrid = [0]
    min_hybrid = [0]
    for i in range(1,len(deaths_total_mean_hybrid)):
        mean_hybrid.append(deaths_total_mean_hybrid[i]-deaths_total_mean_hybrid[i-1])
        max_hybrid.append(deaths_total_max_hybrid[i]-deaths_total_max_hybrid[i-1])
        min_hybrid.append(deaths_total_min_hybrid[i]-deaths_total_min_hybrid[i-1])
    '''
    ...
    '''
    upperbound = np.load(os.path.join(root,this_msa,'age_aware_1.5_upperbound_%s_%s.npy'%(scales[1.5][this_msa][1],this_msa)))
    upperbound_daily = [0]
    for i in range(1,len(upperbound)):
        upperbound_daily.append(upperbound[i]-upperbound[i-1])
        
    lowerbound = np.load(os.path.join(root,this_msa,'age_aware_1.5_lowerbound_%s_%s.npy'%(scales[1.5][this_msa][0],this_msa)))
    lowerbound_daily = [0]
    ...
    alpha = 0.1
    '''
    plt.plot(deaths_daily_total_age_agnostic,label='Meta-population',marker='o',markersize=markersize,color='C0') 
    plt.plot(deaths_daily_total_no_vaccination,label='BD',marker='o',markersize=markersize,color='C1') 
    plt.plot(predicted_deaths_daily,label='Standard SEIR',marker='o',markersize=markersize,color='grey',alpha=0.6)
    plt.scatter(np.arange(len(deaths_daily)),deaths_daily,color='grey',marker='x') 
    plt.plot(deaths_daily_smooth,label='Ground Truth',marker='o',markersize=markersize,color='C2')
    '''
    ...
    # Save the figure
    savepath = os.path.join(saveroot, subroot , 'sup', f'sup_curve_{this_msa}.pdf')
...
# Save figure
savepath = os.path.join(saveroot, subroot , 'sup', f'sup_curve_legend.pdf')
plt.savefig(savepath,bbox_inches = 'tight')

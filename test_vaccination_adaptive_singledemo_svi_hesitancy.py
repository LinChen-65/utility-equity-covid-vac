# python test_vaccination_adaptive_singledemo_svi_hesitancy.py MSA_NAME VACCINATION_TIME VACCINATION_RATIO consider_hesitancy ACCEPTANCE_SCENARIO quick_test
# python test_vaccination_adaptive_singledemo_svi_hesitancy.py Atlanta 31 0.1 True real False
...
for ACCEPTANCE_SCENARIO in ACCEPTANCE_SCENARIO_LIST:
    ...
    if(quick_test=='True'):
        ...
        for policy in policy_list:
            policy = policy.lower()
            if(need_to_save_dict[policy]==True):
                if((policy!='baseline')&('reverse' in policy)):
                    filename = os.path.join(root, MSA_NAME, subroot,

...
if(args.with_comprehensive): # for Fig.4 #20220307
    ...
    if(args.consider_hesitancy): #20220309
...
if(not args.consider_accessibility):
    if(args.consider_hesitancy):
        notation_string = f'acceptance_{args.acceptance_scenario}_'
    else:
        notation_string = ''
else:
    if(args.consider_hesitancy):
        notation_string = f'access_acceptance_{args.acceptance_scenario}_'
    else:
...
for this_msa in msa_name_list:
    ...
    for policy in policy_list:
        ...
        if((policy in ['minority', 'minority_reverse']) & (args.vaccination_ratio in [0.4, 0.56])):
            ...
            #pdb.set_trace()
            list_glob = glob.glob(os.path.join(saveroot, f'comprehensive/vac_results_{args.vaccination_time}d_{args.vaccination_ratio}_{this_recheck_interval}', f'final_deaths_{policy}_{args.vaccination_time}d_{args.vaccination_ratio}_{this_recheck_interval}_*_30seeds_{notation_string}{this_msa}'))
            final_deaths_result_path = list_glob[0]
            
        else:
            final_deaths_result_path = os.path.join(saveroot, f'vac_results_{args.vaccination_time}d_{args.vaccination_ratio}_{recheck_interval_others}', f'final_deaths_{policy}_{args.vaccination_time}d_{args.vaccination_ratio}_{args.recheck_interval}_30seeds_{notation_string}{this_msa}')
        ...
        if(os.path.exists(final_deaths_result_path)):
            ...
            if policy in ['minority', 'minority_reverse']:
                if(args.vaccination_ratio in [0.03,0.05,0.08,0.1,0.13,0.15,0.18,0.2]):
                    exact_name = f'history_D2_{policy}_{args.vaccination_time}d_{args.vaccination_ratio}_{recheck_interval_others}_30seeds_{notation_string}{this_msa}'
                    ...
                    exact_name = f'history_D2_{policy}_{args.vaccination_time}d_{args.vaccination_ratio}_{args.vaccination_ratio/10}_30seeds_{notation_string}{this_msa}'
                ...
                if(args.vaccination_ratio in [0.4,0.56]):
                    history_D2_result_path = os.path.join(root,this_msa,f'vaccination_results_adaptive_{str(args.vaccination_time)}d_{args.vaccination_ratio}_{recheck_interval_others}', f'test_history_D2_{file_name_dict[policy]}_adaptive_{str(args.vaccination_time)}d_{args.vaccination_ratio}_{args.recheck_interval}_30seeds_{notation_string}{this_msa}') #20220308   
                else:
                    history_D2_result_path = os.path.join(root,this_msa,f'vaccination_results_adaptive_{str(args.vaccination_time)}d_{args.vaccination_ratio}_{recheck_interval_others}', f'history_D2_{file_name_dict[policy]}_{str(args.vaccination_time)}d_{args.vaccination_ratio}_30seeds_{notation_string}{this_msa}') #20220307 
            elif(policy=='real_scaled'): #20220308
                history_D2_result_path = os.path.join(root,this_msa,f'vaccination_results_adaptive_{str(args.vaccination_time)}d_{args.vaccination_ratio}_{recheck_interval_others}', f'test_history_D2_{file_name_dict[policy]}_adaptive_{str(args.vaccination_time)}d_{args.vaccination_ratio}_{args.recheck_interval}_30seeds_{notation_string}{this_msa}') #20220308   
                ...
                if('reverse' not in policy):
                    if(args.vaccination_time==31):
                        if(args.vaccination_ratio in [0.05,0.1,0.15,0.2]):
                            if((args.vaccination_ratio==0.1) & (args.consider_hesitancy)): #20220309
                                 exact_name = f'test_history_D2_{file_name_dict[policy]}_adaptive_{str(args.vaccination_time)}d_{args.vaccination_ratio}_{recheck_interval_others}_30seeds_{notation_string}{this_msa}'
                            else:
                                exact_name = f'20210206_history_D2_{file_name_dict[policy]}_adaptive_{args.vaccination_ratio}_{recheck_interval_others}_30seeds_{notation_string}{this_msa}'
                        elif(args.vaccination_ratio in [0.03,0.08,0.13,0.18]):
                            exact_name = f'history_D2_{file_name_dict[policy]}_adaptive_{str(args.vaccination_time)}d_{args.vaccination_ratio}_{recheck_interval_others}_30seeds_{notation_string}{this_msa}' #20220304
                        elif(args.vaccination_ratio in [0.4,0.56]):
                            exact_name = f'test_history_D2_{file_name_dict[policy]}_adaptive_{str(args.vaccination_time)}d_{args.vaccination_ratio}_{recheck_interval_others}_30seeds_{notation_string}{this_msa}' #20220304
                    else:
                        if(args.vaccination_time in [26,36,41]):
                            exact_name = f'20210206_history_D2_{file_name_dict[policy]}_adaptive_{str(args.vaccination_time)}d_{args.vaccination_ratio}_{recheck_interval_others}_30seeds_{notation_string}{this_msa}' #20220306
                        elif(args.vaccination_time in [24,29,34,39]):
                            exact_name = f'history_D2_{file_name_dict[policy]}_adaptive_{str(args.vaccination_time)}d_{args.vaccination_ratio}_{recheck_interval_others}_30seeds_{notation_string}{this_msa}' #20220306
    ...
    if(args.with_comprehensive): #20220307
        ...
        save_path = os.path.join(saveroot, f'gini_table/gini_table_{notation_string}{str(args.vaccination_time)}_{args.vaccination_ratio}_{args.recheck_interval}_{args.num_groups}_{this_msa}_rel2{args.rel_to}.csv')

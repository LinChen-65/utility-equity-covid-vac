# python compare_middle_policies.py --msa_name SanFrancisco  --rel_to Baseline
...
for this_msa in msa_name_list:
    ...
    cbg_ethnic_msa['Minority_Absolute'] = (cbg_ethnic_msa['Sum'] - cbg_ethnic_msa['NH_White']) #20220314
    ...
    order_array = np.arange(5)

    # Age
    policy_list = ['Baseline','Age_Reverse','Age_Target1', 'Age_Target2', 'Age_Target3', 'Age']
    demo_feat_list = ['Elder_Ratio']
    for policy in policy_list:
        #print(f'policy: {policy}')
        final_deaths_result_path = os.path.join(saveroot, f'vac_results_{args.vaccination_time}d_{args.vaccination_ratio}_{args.recheck_interval}', f'final_deaths_{policy.lower()}_{args.vaccination_time}d_{args.vaccination_ratio}_{args.recheck_interval}_30seeds_{this_msa}')
        if(os.path.exists(final_deaths_result_path)):
            #print('Final deaths file already exists.')
            exec(f'final_deaths_{policy.lower()} = np.fromfile(\'{final_deaths_result_path}\')')
            exec(f'final_deaths_{policy.lower()} = np.reshape(final_deaths_{policy.lower()},(args.num_seeds,M))')  
            exec(f'avg_final_deaths_{policy.lower()} = final_deaths_{policy.lower()}.mean(axis=0)') 
        else:
            print('Final deaths file not found. Please check.')
            pdb.set_trace()
        exec("cbg_age_msa['Final_Deaths_%s'] = avg_final_deaths_%s" % (policy,policy.lower()))

    gini_df = make_gini_table(policy_list, demo_feat_list, save_result=False, save_path=None)
    #print(gini_df)
    #print('Utility:\n', stats.spearmanr(order_array,np.array(gini_df.iloc[1])[1:].astype(np.float32)).correlation)
    #print('Age equity:\n',stats.spearmanr(order_array,np.array(gini_df.iloc[3])[1:].astype(np.float32)).correlation)
    #print('Utility:\n', np.round(stats.spearmanr(order_array,np.array(gini_df.iloc[1])[1:].astype(np.float32)).correlation, 3))
    #print('Age equity:\n',np.round(stats.spearmanr(order_array,np.array(gini_df.iloc[3])[1:].astype(np.float32)).correlation, 3))
    print('Utility:\n', np.round(np.corrcoef(order_array,np.array(gini_df.iloc[1])[1:].astype(np.float32))[0,1],3))
    print('Age equity:\n',np.round(np.corrcoef(order_array,np.array(gini_df.iloc[3])[1:].astype(np.float32))[0,1],3))
    #pdb.set_trace()

    #  Income
    policy_list = ['Baseline','Income_Reverse','Income_Target1', 'Income_Target2', 'Income_Target3', 'Income']
    demo_feat_list = ['Mean_Household_Income']
    for policy in policy_list:
        #print(f'policy: {policy}')
        final_deaths_result_path = os.path.join(saveroot, f'vac_results_{args.vaccination_time}d_{args.vaccination_ratio}_{args.recheck_interval}', f'final_deaths_{policy.lower()}_{args.vaccination_time}d_{args.vaccination_ratio}_{args.recheck_interval}_30seeds_{this_msa}')
        if(os.path.exists(final_deaths_result_path)):
    ...
    # Occupation
    policy_list = ['Baseline','Occupation_Reverse','Occupation_Target1', 'Occupation_Target2', 'Occupation_Target3', 'Occupation']
    demo_feat_list = ['EW_Ratio']
    for policy in policy_list:
        #print(f'policy: {policy}')
        final_deaths_result_path = os.path.join(saveroot, f'vac_results_{args.vaccination_time}d_{args.vaccination_ratio}_{args.recheck_interval}', f'final_deaths_{policy.lower()}_{args.vaccination_time}d_{args.vaccination_ratio}_{args.recheck_interval}_30seeds_{this_msa}')
        if(os.path.exists(final_deaths_result_path)):
    ...
    # Minority
    policy_list = ['Baseline','Minority_Reverse','Minority_Target1', 'Minority_Target2', 'Minority_Target3', 'Minority']
    demo_feat_list = ['Minority_Ratio']
    for policy in policy_list:
        #print(f'policy: {policy}')
        final_deaths_result_path = os.path.join(saveroot, f'vac_results_{args.vaccination_time}d_{args.vaccination_ratio}_{args.recheck_interval}', f'final_deaths_{policy.lower()}_{args.vaccination_time}d_{args.vaccination_ratio}_{args.recheck_interval}_30seeds_{this_msa}')
        if(os.path.exists(final_deaths_result_path)):

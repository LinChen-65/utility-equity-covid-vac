...
if(args.consider_hesitancy):
    notation_string = 'acceptance_%s_'%args.acceptance_scenario
else:
    notation_string = ''   
...
for policy in policy_to_compare:
    ...
    if(policy=='no_vaccination'):
        ...
        if(not args.consider_hesitancy):
        ...
        elif(args.consider_hesitancy):
            exec('history_D2_%s = np.fromfile(os.path.join(root,args.msa_name,subroot,\'test_history_D2_%s_adaptive_%sd_%s_%s_30seeds_acceptance_%s_%s\'))' 
                %(policy,policy,str(args.vaccination_time),args.vaccination_ratio,recheck_interval_others,args.acceptance_scenario,args.msa_name))
            exec('history_D2_%s = np.reshape(history_D2_%s,(63,NUM_SEEDS,M))'%(policy,policy))
...
gini_table_no_vaccination = make_gini_table(policy_list=policy_to_compare_rel_to_no_vaccination, demo_feat_list=demo_feat_list, num_groups=args.num_groups, 
...
if(os.path.exists(os.path.join(root, '3cbg_avg_infect_same_%s.npy'%args.msa_name))):
    #print('cbg_avg_infect_same, cbg_avg_infect_diff: Load existing file.')
    cbg_avg_infect_same = np.load(os.path.join(root, '3cbg_avg_infect_same_%s.npy'%args.msa_name))
    cbg_avg_infect_diff = np.load(os.path.join(root, '3cbg_avg_infect_diff_%s.npy'%args.msa_name))
else:
...
S_ratio = SEIR_at_30d[args.msa_name]['S'] / (cbg_sizes.sum())
I_ratio = SEIR_at_30d[args.msa_name]['I'] / (cbg_sizes.sum())
...
while(True):
    ...
    file_savename = os.path.join(root,args.msa_name,subroot,
                                     ...
                                     %(str(args.vaccination_time),args.vaccination_ratio,args.recheck_interval,w1,w2,w3,w4,w5,NUM_SEEDS,notation_string,args.msa_name)
                                     )
    ...
    gini_table_no_vaccination = make_gini_table(policy_list=policy_all_no_vaccination, demo_feat_list=demo_feat_list, num_groups=args.num_groups, 
                                                rel_to='No_Vaccination', save_path=None, save_result=False)
...
file_savename = os.path.join(root,args.msa_name,subroot,
                             ...
                             % (str(args.vaccination_time),args.vaccination_ratio,args.recheck_interval,w1,w2,w3,NUM_SEEDS,notation_string,args.msa_name))
if(os.path.exists(file_savename)):
    print('Result already exists. No need to simulate. Directly load it. Weights: ', weights)  
    ...
    for i in range(int(distribution_time)):
        ...
        cbg_hybrid_msa['Age_Score'] = cbg_hybrid_msa.apply(lambda x : x['Age_Score'] if x['Final_Deaths_Current_Quantile']==args.num_groups-1 else (len(cbg_hybrid_msa)+1), axis=1) # 20210709
        cbg_hybrid_msa['Income_Score'] = cbg_hybrid_msa.apply(lambda x : x['Income_Score'] if x['Final_Deaths_Current_Quantile']==args.num_groups-1 else (len(cbg_hybrid_msa)+1), axis=1) # 20210709
        cbg_hybrid_msa['Occupation_Score'] = cbg_hybrid_msa.apply(lambda x : x['Occupation_Score'] if x['Final_Deaths_Current_Quantile']==args.num_groups-1 else (len(cbg_hybrid_msa)+1), axis=1) # 20210709
